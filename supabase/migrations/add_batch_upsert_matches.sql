-- ============================================
-- Batch upsert matches from API (SECURITY DEFINER)
-- Allows server-side script to upsert without auth
-- ============================================
...
begin
  ...
  select
    ...
    nullif((item->>'goles_local')::int, 0)::int,
    nullif((item->>'goles_visitante')::int, 0)::int,
    item->>'estado',
