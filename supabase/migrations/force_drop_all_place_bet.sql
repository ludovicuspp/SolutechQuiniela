  -- ============================================================
  -- FORCE: Eliminar TODAS las versiones de place_bet + get_bet_multiplier
  -- y recrearlas con el mensaje correcto de "10 minutos".
  -- Motivo: funciones viejas con "12:00 PM" siguen activas.
  -- ============================================================

  -- Paso 1: Eliminar TODOS los overloads de place_bet (dinámico)
  do $$ declare
    r record;
  begin
    for r in
    ...
    loop
      execute format('drop function if exists public.place_bet(%s) cascade', r.params);
      raise notice 'DROPPED place_bet(%s)', r.params;
    end loop;
  end $$;

  -- Paso 2: Eliminar TODOS los overloads de get_bet_multiplier
  do $$ declare
    r record;
  begin
    for r in
      ...
      join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'public'
        and p.proname = 'get_bet_multiplier'
    loop
      execute format('drop function if exists public.get_bet_multiplier(%s) cascade', r.params);
      raise notice 'DROPPED get_bet_multiplier(%s)', r.params;
    end loop;
  end $$;

  -- ============================================================
  -- Paso 3: Recrear get_bet_multiplier
  -- ============================================================
  ...
  -- ============================================================
  -- Paso 4: Recrear place_bet (única versión, mensaje correcto)
  -- ============================================================
  ...
  declare
    ...
    -- p_fecha_partido YA ESTÁ en UTC → restamos 10 min directamente
  ...
  -- ============================================================
  -- Paso 5: Verificación - Debe mostrar SOLO una función
  -- ============================================================
  -- Ejecuta esto DESPUÉS para confirmar:
  -- select proname, pg_get_function_identity_arguments(oid) as params
  -- from pg_proc
  -- where pronamespace = 'public'::regnamespace
  --   and proname in ('place_bet', 'get_bet_multiplier')
  -- order by proname;
