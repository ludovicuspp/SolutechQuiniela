...
-- Corrige place_bet (bug AT TIME ZONE), tipos {ganador,empate,resultado_exacto},
-- multiplicadores planos 2/3/5, ventana 00:00 día anterior → 10 min antes,
-- CHECK constraint y resolve_match_bets con soporte de 'empate'.
-- Ejecutar en Supabase SQL Editor (una sola ejecución).
-- ============================================================

-- 1) Multiplicadores planos (autoritativo server-side)
...
-- 2) CHECK: permitir 'empate', quitar 'diferencia_goles'
...
-- 3) place_bet — empate + ventana 10 min (UTC, sin bug de precedencia)
...
declare
  ...
  -- Ventana: 00:00 (VEN) del día anterior  →  10 min antes del partido
  v_partido_ven        date        := (p_fecha_partido at time zone 'America/Caracas')::date;
  v_ventana_inicio     timestamptz := ((v_partido_ven - 1)::timestamp) at time zone 'America/Caracas';
...
begin
  ...
  -- TIME-LOCK server-side (comparación en UTC)
...
-- 4) resolve_match_bets — soporte de 'empate'
